...
//从后面往前换
		...
		while ( pstLow != Ltail && (((Ltail->pos1)&0xffffffffff) > pivot1 || ( ((Ltail->pos1)&0xffffffffff) == pivot1  && ((Ltail->pos2)&0xffffffffff) >= pivot2 ) ) )
		{
			Ltail = Ltail->prior;
		}
//交换high low	
		...
		pstLow->pos2 = Ltail->pos2;
		Ltail->pos1 = iTmp1;
		Ltail->pos2 = iTmp2;
//从前往后换
		while ( pstLow != Ltail && (((pstLow->pos1)& 0xffffffffff)<pivot1||(((pstLow->pos1)& 0xffffffffff)==pivot1&&((pstLow->pos2)&0xffffffffff)<=pivot2 ) ) )
		{
			pstLow = pstLow->next;
		}
//交换high low
