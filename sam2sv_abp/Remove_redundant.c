...
void  Remove_redundant (struct PosNode **L )
{
	...
	while( (p->next)!=NULL )
	{
		if( ((p->next->pos1&0x3fffffffff)!=(p->pos1&0x3fffffffff)) )//'|| ((p->next->pos2&0xffffffffff)!=(p->pos2&0xffffffffff)) )
			p=p->next;
		else
		{
			if(((p->next->pos1)&0xffffff0000000000))
				p->pos1 = p->pos1 + 0x10000000000;
				//p->pos1 = p->pos1 + ((p->next->pos1)&0xffffff0000000000) ;
			else
...
void gather_L( struct PosNode **L , int m_gather_size, int set_pos)
{
	...
	shan_en=0;
			...
			//		printf("pos1: %lld\t%lld<10 ?\n", (q->next->pos1)&0xffffffff,(p_pos1&0xffffffff));
				if((((q->next->pos1)&0x3fffffffff)-(p_pos1&0x3fffffffff)<=m_gather_size))
				{
					shan_en=1;
			...
			//		printf("%lld\t%lld<10 ?\n", (q->next->pos2)&0xffffffff,(p_pos2&0xffffffff));
				...
				//	{
					r=q->next;
					p_pos1=p_pos1+((q->next->pos1)&0xffffff0000000000);
					p_pos2=p_pos2+((q->next->pos2)&0xffffff0000000000);
