...
void  merge_pos(struct PosNode **L , int set_pos)
{
	...
	while( (p->next)!=NULL )
	{
		if(    ( (((p->next->pos1)&0xffffff0000000000)>>40) + (((p->next->pos2)&0xffffff0000000000)>>40 ))< set_pos  )
		{
			p=p->next;
			if(p->prior!=NULL)
				p->prior->next=p->next;
...
void filer_L( struct PosNode **L , int Area)
{
	...
	while( p )
	{
		...
		p_pos1=p->pos1;
		p_pos2=p->pos2;
		while( q->next && (((q->next->pos1)&0x3fffffffff) - (p_pos1&0x3fffffffff) <=Area) )//&& (((q->next->pos2)&0xffffffffff) - (p_pos2&0xffffffffff) <=Area) )
		{
			if(( ((q->pos1)&0xffffff0000000000) + ((q->pos2)&0xffffff0000000000) ) > ((q->next->pos1)&0xffffff0000000000) + ((q->next->pos2)&0xffffff0000000000)  )
			{
				r=q->next;
				if(r->next!=NULL)
				{	
